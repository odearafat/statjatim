...
import com.bps_jatim_3500.statistik_jatim.model.Brs;
import com.bps_jatim_3500.statistik_jatim.model.BrsItem;
...
public class BrsFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        mCurrentLayoutManagerType = BrsFragment.LayoutManagerType.LINEAR_LAYOUT_MANAGER;

        if (savedInstanceState != null) {
            // Restore saved layout manager type.
            mCurrentLayoutManagerType = (BrsFragment.LayoutManagerType) savedInstanceState
    ...
    public void getListData(){
        ...
        //Callback
        call.enqueue(new Callback<Brs>() {
            @RequiresApi(api = Build.VERSION_CODES.N)

            @Override
            public void onResponse(Call<Brs> call, Response<Brs> response) {
                if(!response.isSuccessful()){
                    getResult="Code : "+response.code();
                    return;
                }
                Brs brs=response.body();

                //Cast to Page
                Page page=new Gson().fromJson(brs.getData().get(0).toString(), Page.class);
            ...
            @Override
            public void onFailure(Call<Brs> call, Throwable t) {
                getResult="Gagal";
                progressBar.setVisibility(View.GONE);
    ...
    public void performPagination(){
        ...
        call.enqueue(new Callback<Brs>() {
            ...
            public void onResponse(Call<Brs> call, Response<Brs> response) {
                ...
                //Cast to List of Publikasi Item
                List<BrsItem>BrsItem2 = stringToArray(new Gson().toJson(brs.getData().get(1)), BrsItem[].class);

                //publikasiItems.addAll(publikasiItem2);
                brsAdapter.addBrsItemList(BrsItem2);
    ...
    private void setRecyclerViewLayoutManager(BrsFragment.LayoutManagerType layoutManagerType) {
        ...
        switch (layoutManagerType) {
            case GRID_LAYOUT_MANAGER:
                ...
                mCurrentLayoutManagerType = BrsFragment.LayoutManagerType.GRID_LAYOUT_MANAGER;
                break;
